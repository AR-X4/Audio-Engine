...
{
	HandleManager* pMan = HandleManager::GetInstance();
	assert(pMan);
	...
	if (status == Handle::Status::SUCCESS)
	{
		...
		{
			// Is ID valid?
			status = pMan->IsHandleValid(handle);
...
Handle::Status HandleManager::IsHandleValid(const Handle& handle)
{
	Handle::Status status = Handle::Status::HANDLE_ERROR;
...
Handle::Status HandleManager::IsHandleAndRangeValid(const Handle& handle)
{
	Handle::Status status = Handle::Status::HANDLE_ERROR;
